...
/// Поджог: шанс наложить урон по времени.
class IgniteOnHitBuff extends LevelBasedBuff {
  IgniteOnHitBuff({
    required super.rarity,
    required super.maxLevel,
    required super.levels,
  }) : super(id: 'buff_ignite_on_hit');
  ...
  void onEvent(PlayerComponent owner, CombatEvent event) {
    ...
    final v = levelValues();
    final procChance = altarNum(v, 'procChance', 0.12);
    if (owner.game.rng.nextDouble() > procChance) return;

    final durationSec = altarNum(v, 'durationSec', 2.5);
    final totalDamagePct = altarNum(v, 'totalDamagePct', 0.18);
